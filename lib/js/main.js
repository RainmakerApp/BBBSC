(function() {
  ...
  $(window).load(function() {
    ...
    if (hash.indexOf('/fundraiser/create') > -1) {
      ...
      if (isInt(step)) {
        ...
        $('.step[data-step="1"]').show().siblings('.step').hide();
        $('.wrapper__forms-nav').slideDown('150');
        return $('.wrapper__title').slideUp('150');
      } else {
  ...
  $(window).on('hashchange', function() {
    ...
    if (window.location.href.indexOf('about') > -1) {
      ...
      if (hash === 'history') {
        ...
        $('#history').addClass('active').siblings('.tab').removeClass('active');
        $(elem).fadeIn('150').siblings('.tab').removeClass('active');
        return $('html, body').animate({
          scrollTop: $('#' + hash).offset().top
        }, 1000);
      } else {
        $('[data-target="' + hash + '"]').addClass('active').siblings('.tab').removeClass('active');
        $(elem).fadeIn('150').siblings('.pane').hide();
        return $('html, body').animate({
          scrollTop: $('#' + hash).offset().top
        }, 1000);
      }
  ...
  $('.edit-photos-trigger').on('click', function() {
    return $('#banner').addClass('active');
  });
  ...
  fundraiserModal = function(team) {
    ...
    if (Modernizr['cssfilters']) {
      ...
      if (typeof user === 'undefined') {
        $('[data-step="0"]').fadeIn('150').siblings('.step').fadeOut('150');
      } else {
        $('[data-step="1"]').fadeIn('150').siblings('.step').fadeOut('150');
      }
    } else {
      $('body').addClass('create-fundraiser-no-filters');
      $(canvas).css({
        'max-height': height + 'px',
        'overflow': 'scroll'
      });
      $('.close-modal').on('click', function() {
        $('body').removeClass('create-fundraiser-no-filters');
        $(canvas).css({
          'max-height': 'none',
          'overflow': 'visible'
        });
  ...
  $('.delete').each(function() {
    return $(this).on('click', function() {
      ...
      return $.ajax({
        ...
        success: function() {
          console.log('Success!');
          return click.parents('.item').fadeOut('150', function() {
  ...
  $('.nav a').on('click', function() {
    ...
    if (className === 'back') {
      ...
      return $('.nav a.next').text('Next').removeClass('submit');
      ...
      if (current === max) {
        return $('.nav a.next').text('Submit').addClass('submit');
      }
    }
  });
  ...
  $('.show__form').on('click', function() {
    ...
    if ($(this).hasClass('pikes')) {
      return $('input[name="region"]').val('Pikes Peak');
    } else if ($(this).hasClass('denver')) {
      return $('input[name="region"]').val('Denver');
    }
  });

  $('.glass-form__wrapper').on('click', function() {
    return $('body').removeClass('show student referral pikes denver volunteer');
  });

  $('.show__form__v').on('click', function() {
    $('body').addClass('show volunteer');
    if ($(this).hasClass('pikes')) {
      return $('body').addClass('pikes');
    } else if ($(this).hasClass('denver')) {
      return $('body').addClass('denver');
    }
  });
  ...
  $('.tab input').on('keyup keydown focus blur change', function() {
    var empty;
    if ($(this).parents('.tab').hasClass('in')) {
      empty = $(this).parents('.in').find('input.required').filter(function() {
        return this.value === '';
      });
      if (empty.length) {
  ...
  $('#form').on('submit', function(e) {
    ...
    $('#form input[type="text"]').each(function() {
      return form_data += '<strong>' + $(this).data('label') + ':</strong> ' + $(this).val() + '<br>';
    });
    $('#form textarea').each(function() {
      return form_data += '<strong>' + $(this).data('label') + ':</strong> ' + $(this).val() + '<br>';
    });
    $('#request-body').val(form_data);
    return console.log($('#request-body').val());
  });
